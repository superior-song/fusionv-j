...
jQuery.fn.extend({
	...
	cs : function(url, reqParamName, reqParamValue, isHideNoOptionSelect, selectLen) {
		...
		this.change(function(){
			// Fixed if select empty value then subElement not change
			// date 2013-11-20		
			var _attrSubElement = $(this).attr("subElement");
			var _attrVal = $(this).val();
			var $_subElement = $("#" + attrSubElement);
			if(null == _attrVal || "" == _attrVal){
				if (null != _attrSubElement) {
					hideSubElementsValue($_subElement);
				}
			}
		...
		function hideSubElementsValue($select){
			$select.empty();
			var attrSubElement_ext= $select.attr("subElement");
			var $subElement_ext = $("#" + attrSubElement_ext);
			if (null != attrSubElement_ext) {
				hideSubElementsValue($subElement_ext);
			}
		}
	}
});
